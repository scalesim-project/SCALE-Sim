...
class Bagel_sim():
    ...
    def read_from_json(self, cfg_path=None):
        if cfg_path is None:
            ...
            cfg_path = os.path.join(base, "topologies", "bagel", "config_ours_sparse_ffn.json")
    ...
    def run_gen_text(self):
        ...
        input_cycles = self.run_sim_once(kv_length=0, is_gen_text=True, part='qkv', config=self.config_int8)
        ...
        output_cycles = self.run_sim_once(kv_length=0, is_gen_text=True, part='omap', config=self.config_int8)
        ...
        for step in range(0, self.gen_text_len, self.sample_rate):
            ...
            attn_single_cycles = self.run_sim_once(kv_length=kv_len, is_gen_text=True, part='attn', config=self.config_int8)
