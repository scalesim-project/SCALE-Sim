...
[whole]   a0*GEMM + a1*noncompute + C             -> COMPENSATION_BY_GEN[<GEN>] in latency_model/compensation.py
[ops]     non-compute per-op models               -> scalesim/latency_model/ml_ml_model/<gen>/*.pkl
```
...
```bash
cd scalesim/latency_model/linear_model                     # all GEMM calibration = gemm_calibration.py
python3 gemm_calibration.py sample --out shapes_stratified.csv   # ~1260, region-balanced
PJRT_DEVICE=TPU python3 gemm_calibration.py collect \
    --shapes shapes_stratified.csv \
    --out   gemm_fusion_strat_<gen>.csv --iters 12              # golden `fusion` time
python3 gemm_calibration.py fit --data gemm_fusion_strat_<gen>.csv   # prints the 12-region table
```
- The fit script's region scheme = `(foldK>1)*4+(foldN>1)*2+(foldM>1)`. To get the
  **12-region** scheme (with the 2nd M bar at `foldM>=6`) used for v4, apply the same
  `+ (foldM>=6)*8` split when fitting (see how v4's table was produced in SCALE-Sim_TPU/calibration_data_backup/RESULTS.md
...
## 2. Non-compute per-op models  (-> `ml_model/<gen>/*.pkl`)
All in `model/op_calibration.py` (`collect` / `train` / `reshape-median`).
PURE device-span method: `collect` reads each op's real kernel time off the xprof
device timeline = **sum of the inner spans nested in the `jit_*` wrapper, EXCLUDING the
wrapper** (the wrapper is the ~10 us per-launch floor; reading it as the kernel was the
old bug that inflated every op ~10 us). Uniform rule across all 24 ops, no per-op
special-casing (composite ops like `batch_norm_training` just sum their fusions).
```bash
cd scalesim/latency_model/ml_model
...
PJRT_DEVICE=TPU python3 op_calibration.py collect --ops $OPS \
    --outdir datasets_pure_<gen>_fixed
...
python3 op_calibration.py train --datadir datasets_pure_<gen>_fixed --outdir <gen>  # -> ml_model/<gen>/*.pkl
...
python3 op_calibration.py reshape-median --model-dir <gen> \
    --dataset datasets_pure_<gen>_fixed/reshape_dataset.csv
```
...
{1,8,16,64,128,256,512,1024}` before `train`, or collect with `llm_frac=1.0`.)
`NonComputeLatencyPredictor` auto-selects `ml_model/<gen>/` from the config's
`TimeLinearModel:` key (falls back to tpuv4 if absent), so the new models go live once written.

## 3. Level-2 batch reduction  (-> `tpu.py` batch-reduction coeffs)
```bash
cd scalesim/latency_model/linear_model
PJRT_DEVICE=TPU python3 gemm_calibration.py batch     # -> batch_reduction.csv (einsum vs batch*single)
```
Fit `R = u + (1-u)/B`, `u = nt^p/(nt^p + c)`, `nt = ceil(M/128)*ceil(N/128)` (the fit
loop is in the SCALE-Sim_TPU/calibration_data_backup/RESULTS.md "batch" section / fit_piecewise helpers). **Wire in:** a
...
```bash
cd scalesim/latency_model/calibration
# 4a. whole-model truth (torch.compile device-busy) for the 3 LLMs x seq x batch:
PJRT_DEVICE=TPU python3 measure_truth.py llm --gen <gen>       # -> e2e_device_truth_<gen>.csv
...
PJRT_DEVICE=TPU python3 measure_truth.py tiny                  # prints tiny device-busy us
# 4c. fit (CPU; uses the in-repo calib_mlir + ml_model/<gen> from step 2):
...
c_n:0, C0_forward:<C0>, C1_per_gemm:0.0}` in `latency_model/compensation.py`. Batch-1 only
