...
# SCALE-Sim_TPU/calibration_data_backup/RESULTS.md.
#
...
# (SCALE-Sim_TPU/calibration_data_backup/gemm_fusion_strat.csv, gemm_calibration.py fit); held-out
...
# not the loop-method marginal signal (which ran ~1.4x higher). See SCALE-Sim_TPU/calibration_data_backup/RESULTS.md.
TPUV4_REGION_TABLE = {
...
# SCALE-Sim_TPU/e2e_work/compensation/ (gemm_calibration.py batch, batch_reduction.csv).
...
def tpuv4_linear_model(cycles, s_row=1, s_col=1, t_time=1, M=None, N=None, K=None):
    ...
    single global fit A0*cycles + B0 (G0). Fit metadata: see
    SCALE-Sim_TPU/calibration_data_backup/fit_outputs/tpuv4_coeffs.json.
...
# v6e 12-region fusion table (calibration/README.md step 1, done 2026-06-25).
...
# (SCALE-Sim_TPU/calibration_data_backup/gemm_pure_master_tpuv6e.csv, gemm_calibration.py fit);
...
def tpuv6e_batch_reduction(batch, M, N, K):
    ...
    Calibrated on a v6e batch x shape xprof sweep (gemm_calibration.py batch,
...
def tpuv6e_linear_model(cycles, s_row=1, s_col=1, t_time=1, M=None, N=None, K=None):
    ...
    v6e (SCALE-Sim_TPU/calibration_data_backup/gemm_pure_master_tpuv6e.csv, gemm_calibration.py fit
    .py); held-out fusion-time MAPE 11.7% (vs 27.0% single G_roof). See
    TPUV6E_REGION_TABLE and SCALE-Sim_TPU/calibration_data_backup/RESULTS.md.
