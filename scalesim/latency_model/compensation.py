"""
Whole-model (end-to-end) compensation for the TPU latency model.

The per-op STANDALONE latencies (GEMM from linear_model, non-compute from ml_model)
sum to far more than real fused device time; this layer corrects the sum to fused
DEVICE-BUSY time, per generation:

    T = a0*Sum(GEMM) + a1*Sum(non-compute) + C_forward

The constants are PRODUCED by calibration/fit_compensation_pure.py and CONSUMED by
total_time_report.py (which assembles the report). Also holds the per-op eager
DISPATCH cost (wall-clock model). This module is just the calibrated model constants
...
COMPENSATION_BY_GEN = {
    ...
    #   PURE per-op models (scalesim/latency_model/ml_model/tpuv4 = xprof single-op spans, fixed inner-
    ...
    # v6e: same pure-model pipeline as v4 (PURE per-op models in ml_model/tpuv6e,
    ...
    # re-run `op_calibration.py reshape-median` on ml_model/tpuv6e if porting.
    # Batch>1 occupancy NOT modelled.
