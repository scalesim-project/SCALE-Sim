...
- `op_calibration.py` — one script (`collect` / `train` / `reshape-median` stages) to
  measure and train the models on a TPU (below).
...
- **Measurement (§f):** `op_calibration.py collect` reads the single-op kernel span
...
- **`reshape` is a constant = the median (~7µs)**, NOT a shape model. reshape latency
  ...
  the HGBR reshape). See `total_time_report.py` and SCALE-Sim_TPU/calibration_data_backup/RESULTS.md.
...
To build the models for a different TPU, re-run `op_calibration.py` **on that TPU** and
write the `.pkl`s into a new `ml_model/<generation>/` directory.
...
All in `op_calibration.py` (`collect` / `train` / `reshape-median`):
```bash
cd scalesim/latency_model/ml_model

# 1. measure PURE device kernel latency per op on the target TPU (resumable; one CSV/op)
PJRT_DEVICE=TPU python3 op_calibration.py collect \
    --ops add subtract multiply maximum minimum divide negate rsqrt exponential \
          ...
          sine cosine convert compare and select batch_norm_training   # -> <op>_dataset.csv

# 2. train one HistGBR per dataset -> .pkl with {model, op_name, metadata}
python3 op_calibration.py train --datadir <datasets> --outdir <generation>

# 3. reshape isn't shape-predictable -> override it with the median constant
python3 op_calibration.py reshape-median --model-dir <generation> \
    --dataset <datasets>/reshape_dataset.csv
```
...
them automatically — just point it at the new `ml_model/<generation>/` dir (or make it
the default). To add a brand-new op: add it to the `OPS`/`build()` tables in
`op_calibration.py` (`_build_ops`), then collect + train.
...
| 3 | collect 25 ops | `PJRT_DEVICE=TPU python3 op_calibration.py collect --ops … --n 1000 --outdir datasets_tpuv6e` (run in 5 op-batches for crash-safety) | `calibration/datasets_tpuv6e/<op>_dataset.csv` × 25 | ✅ done |
| 4 | train | `python3 op_calibration.py train --datadir datasets_tpuv6e --outdir ../tpuv6e` | `model/tpuv6e/*.pkl` × 25 | ✅ done |
...
The §a labels are the **pure device span**, read **directly** off the trace by
`op_calibration.py collect`: it compiles the op (`jit().lower().compile()`), runs it
...
`op_calibration.py train` (same `[d0,d1,d2,size,log2_size,latency_us]` schema; `wall/host`
are extra columns).
...
> old ~10µs-floor labels), kept for history; re-collect v6e per the calibration/README.md
> to refresh them.

Rebuilt all 25 v6e per-op models on **trace-authoritative pure-kernel labels**
(`op_calibration.py collect`, xprof device-span, n=300/op, iters=12; data in
`calibration/datasets_pure_tpuv6e/*_pure_dataset.csv`), trained with the same
`op_calibration.py train` recipe and written to **`model/tpuv6e_pure/`**.
