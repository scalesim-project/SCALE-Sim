...
class Bagel_sim():
    ...
    def read_from_json(self, cfg_path=None):
        if cfg_path is None:
            ...
            cfg_path = os.path.join(base, "topologies", "janus", "config_sdma_janus_GenEval.json")
    ...
    def run_sim_once_comp(self, kv_length=0, is_gen_text=False, part='all', sparsity=1, image_input=0):
        cycle_result = 0
    ...
    def run_gen_image(self):
       ...
       # 三个不同KV配置的图像生成
        ...
        prefetch_cycles = self.run_sim_once_comp(kv_length=0, is_gen_text=False, part='prefetch', sparsity=self.sparsity, image_input=input_normal)
        ...
        for config_name, kv_len, input_len in kv_configs:
            ...
            input_cycles = self.run_sim_once_comp(kv_length=0, is_gen_text=False, part='qkv', sparsity=self.sparsity, image_input=input_len)
            self._append_to_log(f"Text generation - End qkv mapping, total cycles:{input_cycles}")
            config_cycles_iter += input_cycles

            self._append_to_log(f"Image generation - Start attn")
            attn_single_cycles = self.run_sim_once_comp(kv_length=kv_len, is_gen_text=False, part='attn', sparsity=self.sparsity, image_input=input_len)
            ...
            output_cycles = self.run_sim_once_comp(kv_length=0, is_gen_text=False, part='omap', sparsity=self.sparsity, image_input=input_len)
            ...
            ffn_up_cycles = self.run_sim_once_comp(kv_length=0, is_gen_text=False, part='ffn_up',sparsity=self.sparsity, image_input=input_len)
