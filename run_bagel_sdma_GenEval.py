...
class Bagel_sim():
    ...
    def read_from_json(self, cfg_path=None):
        if cfg_path is None:
            ...
            cfg_path = os.path.join(base, "topologies", "bagel", "config_sdma_GenEval.json")
    ...
    def run_gen_image(self):
       ...
       # 三个不同KV配置的图像生成
        kv_configs = [
            ("full_cache", self.kv_cache_init + self.gen_text_len + self.image_input_len),
            ("without_text", self.kv_cache_without_text + self.gen_text_len + self.image_input_len)
        ]
